...
import deleteTasks from "../../api/http/data/tasks/delete-task"
  ...
  return (
    <section id="task-container">
      ...
      <ul className="task-receiver">
      {
       data.selectedTasks 
       ? 
       (data.selectedTasks && !data.error) && data.selectedTasks.map((task: ITask) => {
        return (
          <>
            {task && typeof task !== 'string' ?
                  <li key={task.task_id} className="active-task">
                    <h1 className="active-task-day">{(Number(task.task_day) >= 1 && Number(task.task_day) <= 9 )?'0':''}{task.task_day}/{(Number(task.task_month) <= 8 )?'0':''}{Number(task.task_month) + 1}</h1>
       ...
       : ( data.tasks && !data.selectedTasks && !data.error ) && data.tasks.map((task: ITask) => {
          return (
            <>
              {task && typeof task !== 'string' ?
                    <li key={task.task_id} className="active-task">
      ...
      }
      {
        ...
        <li className="active-task">
          <h1 className="active-task-day">{Number(data.error.task_day) < 10 ? '0' : ''}{data.error.task_day}/{Number(data.error.task_month) <= 8 ? '0' : ''}{Number(data.error.task_month) + 1}</h1>          
          <div style={{background: 'red'}} className="task-divisor-bar"></div>
          <div className="active-task-about">
            <h1>Sem Tarefas</h1>
            <h2>{data.error.error}</h2>
          </div>
        </li>
      ) : ''
      }

      {((!data.tasks || data.tasks.length === 0) && (!data.selectedTasks || data.selectedTasks.length === 0))  && !data.error
      ? (
        <li className="active-task">
