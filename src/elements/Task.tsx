...
import { ITask } from '../http/data/tasks/services/tasks-interfaces'
  ...
  return (
    <section id="task-container">
      ...
      <ul className="task-receiver">
      {
       data.selectedTasks 
       ? 
       (data.selectedTasks && !data.selectedTasks.error) && data.selectedTasks.map((task: ITask) => {
        return (
          <>
       ...
       : ( data.tasks && !data.tasks.error ) && data.tasks.map((task: ITask) => {
          return (
            <>
              {task && typeof task !== 'string' ?
                    <li key={task.task_id} className="active-task">
